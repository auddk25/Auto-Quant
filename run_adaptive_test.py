"""run_adaptive_test.py — Compare adaptive CBBI strategies.

Usage: uv run run_adaptive_test.py
"""
...
def main():
    print("=" * 80)
    print("Adaptive CBBI Strategy Comparison")
    print("=" * 80)

    strategies = [
        ("MtfTrendCbbiMomentumEnsemble", "Ensemble (baseline)"),
        ("MtfTrendCbbiAdaptive", "Adaptive (ATR-based exit)"),
        ("MtfTrendCbbiMarketFilter", "Market Filter (30d return)"),
    ]
    ...
    print(f"{'Metric':<16} {labels[0]:<20} {labels[1]:<24} {labels[2]:<24}")
    print("-" * 84)
    ...
    for metric_name, key in [("Profit %", "profit_pct"), ("Max DD %", "max_dd_pct"),
        ...
        for j, v in enumerate(vals):
            if key == "trades":
                line += f"{v:<20}" if j == 0 else f"{v:<24}"
            elif key == "profit_factor":
                line += f"{v:<20.2f}" if j == 0 else f"{v:<24.2f}"
            else:
                line += f"{v:<20.1f}" if j == 0 else f"{v:<24.1f}"
        print(line)
