"""run_multi_tf.py — Compare ensemble vs multi-timeframe strategies.

Usage: uv run run_multi_tf.py
"""
...
def main():
    print("=" * 70)
    print("Ensemble vs Multi-Timeframe Strategy Comparison")
    print("=" * 70)

    strategies = [
        ("MtfTrendCbbiMomentumEnsemble", "Ensemble (3-variant voting)"),
        ("MtfTrendCbbiMultiTF", "Multi-TF (1h entry + 4h trend)"),
        ("MtfTrendCbbiMultiTF2", "Multi-TF2 (4h base timeframe)"),
    ]
    ...
    print("Comparison")
    print("=" * 70)
    headers = ["Metric"] + [label for _, label in strategies]
    print(f"{headers[0]:<20} {headers[1]:<18} {headers[2]:<18} {headers[3]:<18}")
    print("-" * 74)

    m1 = results[strategies[0][0]]
