"""MtfTrendCbbiRsiFusion — CBBI + RSI Fusion Strategy

Combines CBBI momentum (proven) with RSI as additional filter.

Entry logic:
- CBBI momentum > 0 (fear subsiding)
- CBBI < 0.65 (not euphoric)
- EMA100 > EMA200 (trend up)
- RSI < RSI_ENTRY (not overbought, avoid buying at top)

Exit logic:
- CBBI momentum < -0.02 (confidence falling)
- OR CBBI > 0.80 (extreme greed)
- OR EMA100 < EMA200 (trend broken)
- OR RSI > RSI_EXIT (overbought exit signal)
...
class MtfTrendCbbiRsiFusion(IStrategy):
    INTERFACE_VERSION = 3
    timeframe = "1h"
    can_short = False
    max_open_trades = 1
    ...
    # ---- RSI Parameters (new) ----
    RSI_PERIOD = 14
    RSI_ENTRY = 70      # Don't enter when RSI > 70 (overbought)
    RSI_EXIT = 75       # Exit when RSI > 75 (strong overbought)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        rsi_ok = dataframe["rsi"] < self.RSI_ENTRY
        volume_ok = dataframe["volume"] > 0

        dataframe.loc[
            fear_subsiding & not_euphoric & trend_ok & rsi_ok & volume_ok,
            "enter_long"
        ] = 1
        return dataframe
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        rsi_overbought = dataframe["rsi"] > self.RSI_EXIT

        dataframe.loc[
            (confidence_falling | extreme_greed | trend_broken | rsi_overbought) & (dataframe["volume"] > 0),
            "exit_long"
        ] = 1
        return dataframe
